...
class row_questions(row_questionsTemplate):
  def __init__(self, **properties):
    ...
    # This ensures each question has its own independent radio button group
    group_name = f"dim_q_{self.item['question_id']}_s_{self.item['sample_number']}"
    ...
    if self.item.get('pass_fail') == 'Pass':
      ...
      self.text_area_notes.visible = True  # Show notes for failed items
  ...
  def image_fl_change(self, file, **event_args):
    """Handle file upload changes for dimension check photos"""
    if file:
      # Store the new photo
  ...
  def get_result(self):
    """
    Collect and return the current state of this question row.
    
    Returns:
        Dictionary containing:
        - question_id: The question identifier
        - pass_fail: 'Pass', 'Fail', 'NA', or None
        - notes: Text notes (only if Fail)
        - photo: Uploaded photo file (if any)
    """
    pass_fail = None
