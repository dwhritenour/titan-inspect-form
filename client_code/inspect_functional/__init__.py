# inspect_functional form - Functional Check Interface
# This form handles the functional check process for incoming materials
...
from anvil import *
import anvil.server
import validation_functional  # Custom validation module for form validation

class inspect_functional(inspect_functionalTemplate):
  """
    Main form for conducting functional checks on product samples.
    
    This form:
    - Displays functional check questions specific to a product series
    - Allows navigation through multiple samples
    - Collects pass/fail responses, notes, and photos for each question
    - Saves all functional check results to the database when complete
    """

  def __init__(self, **properties):
    """
        Initialize the functional check form with inspection details.
    ...
    self.sample_results = {}  # Dictionary to store all functional check results
  ...
  def setup_inspection(self):
    ...
    # Server call to get all active functional check questions for this product series
    self.questions = anvil.server.call('get_functional_questions', self.product_series)

    # Check if questions were found
    if not self.questions:
      alert(f"No functional check questions found for series: {self.product_series}")
      return
  ...
  def load_questions_for_sample(self):
    ...
    for question in self.questions:
      ...
      item = {
        ...
        'question_text': question['question_text'],  # e.g., 'Screen Present & Seated?'
  ...
  def save_current_sample(self):
    ...
    # ===== DEBUG OUTPUT =====
    print(f"=== SAVING FUNCTIONAL SAMPLE {self.current_sample} ===")
    ...
    print(f"Found {len(components)} functional question components")
  ...
  def button_next_click(self, **event_args):
    """
        ...
        - Completes the functional check (if on last sample)
        """
    # ===== VALIDATION CHECK =====
    ...
    print(f"=== FUNCTIONAL NEXT BUTTON CLICKED - Sample {self.current_sample} of {self.sample_size} ===")
    ...
    if self.current_sample < self.sample_size:
      ...
      # This is the last sample - complete the functional check
      print("=== LAST SAMPLE - CALLING COMPLETE FUNCTIONAL CHECK ===")
      self.complete_inspection()

  def complete_inspection(self):
    """
        Save all functional check results to the database.
        ...
        """
    print("=== COMPLETE FUNCTIONAL CHECK CALLED ===")
    ...
    try:
      ...
      result = anvil.server.call(
        'save_functional_inspection_results',  # Server function name
      ...
      # ===== HANDLE RESPONSE =====
      if result['success']:
        alert(f"Functional check saved: {result['message']}")
  ...
  def validate_before_navigation(self):
    ...
    """
    print("=== VALIDATING FUNCTIONAL INSPECTION BEFORE NAVIGATION ===")
    ...
    return validation_functional.validate_before_complete(self)
